...
architecture behavioral of VHD_axis_i2s2 is
    ...
    signal tx_data_l_shift : std_logic_vector(BIT_WIDTH_G-1 downto 0);
    signal tx_data_r_shift : std_logic_vector(BIT_WIDTH_G-1 downto 0);

    signal din_sync : std_logic;
    signal din_sync_shift : std_logic_vector(2 downto 0);

    signal rx_data_l_shift : std_logic_vector(BIT_WIDTH_G-1 downto 0);
    signal rx_data_r_shift : std_logic_vector(BIT_WIDTH_G-1 downto 0);
...
begin
    ...
    begin
        if reset = ACTIVE then
            ...
            if count = 7 then
                tx_data_l_shift <= tx_data_l(31 downto 31-BIT_WIDTH_G+1);
                tx_data_r_shift <= tx_data_r(31 downto 31-BIT_WIDTH_G+1);
                ...
                if count(8) = '1' then --lrck
                    ...
                    tx_data_l_shift <= tx_data_l_shift(BIT_WIDTH_G-2 downto 0) & '0';
                else
                    --tx_data_r_shift <= tx_data_r_shift(22 downto 0) & '0';
                    tx_data_r_shift <= tx_data_r_shift(BIT_WIDTH_G-2 downto 0) & '0';
            ...
            if count(7 downto 3) >= 1 and 
               count(7 downto 3) <= 24 then
                ...
                if count(8) = '1' then
                    ...
                    tx_sdout <= tx_data_l_shift(BIT_WIDTH_G-1);
                else
                    --tx_sdout <= tx_data_r_shift(23);
                    tx_sdout <= tx_data_r_shift(BIT_WIDTH_G-1);
                end if;
    ...
    --until the following process below registers it.
            ...
            if count(2 downto 0) = 3 and    --"011"
               ...
               count(7 downto 3) <= 24 then --"11000"
                ...
                if count(8) = '1' then
                    ...
                    rx_data_r_shift <= rx_data_r_shift(BIT_WIDTH_G-2 downto 0) & din_sync;
                else 
                    --rx_data_l_shift <= rx_data_l_shift(22 downto 0) & din_sync;
                    rx_data_l_shift <= rx_data_l_shift(BIT_WIDTH_G-2 downto 0) & din_sync;
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if reset = ACTIVE then
            ...
            if count = EOF and rx_m_valid_out = '0' then
                rx_data_r <= rx_data_r_shift & ZEROS;
                rx_data_l <= rx_data_l_shift & ZEROS;
